...
class Router {
            ...
            create(contextHandler) {
                ...
                return function operation(input, callback) {
                    ...
                    requestHandler(context.url, context.method, context.input, (err, payload, status, headers) => {
                        ...
                        context.callback(err, payload, status, headers);
                    });
                };
            }
        };
    ...
    static reduceInputProperties(router, routeOptions) {
        ...
        return inputProperties.concat(routeOptions.inputProperties).filter(inputProperties => {
            ...
            inputTypes.forEach(inputType => {
                ...
                schema.properties[inputType] = Object.assign({
                    type: 'object',
                    properties: {},
                    required: []
    ...
    static getFinalInput(reducedInputs, input) {
        ...
        const reducedInputsClone = JSON.parse(JSON.stringify(reducedInputs));
        ...
        Object.keys(input).forEach(field => {
            ...
            Object.assign(finalInput[field], Object.assign({}, input[field]));
        });
    ...
    static reduceResponses(router, routeOptions) {
        ...
        const responses = Object.assign.apply(null, [{}].concat(router.responses.concat(Router.convertSuccessAndErrorToResponse(routeOptions.success, routeOptions.error))));
    ...
    static reduceAjvOptions(router, routeOptions) {
        ...
        return Object.assign.apply(null, [{}].concat(router.ajvOptions.concat(routeOptions.ajvOptions)));
    }
    ...
    static convertSuccessAndErrorToResponse(success, error) {
        ...
        Object.keys(success).forEach(status => {
            ...
            if (isNaN(status)) {
                throw new Error(`Success response status "${status}" is not a valid HTTP status code.`);
            }
        ...
        Object.keys(error).forEach(status => {
            ...
            if (isNaN(status)) {
                throw new Error(`Error response status "${status}" is not a valid HTTP status code.`);
            }
