        ...
        operations[routeDefinition.name] = {
            ...
            create(contextHandler) {
                ...
                return function operation(input, callback) {
                    ...
                    executeEvent(routeDefinition.events, 'onRequestStart', [context]);
                    ...
                    if (invalid) {
                        ...
                        executeEvent(routeDefinition.events, 'onRequestError', [requestError, context]);
                        executeEvent(routeDefinition.events, 'onError', [requestError, context]);
                        executeEvent(routeDefinition.events, 'onRequestEnd', [requestError, context]);
                    ...
                    requestHandler(context.url, context.method, context.input, (err, payload, status, headers) => {
                        ...
                        if (!err) {
                            ...
                            if (response && !response.validate(payload)) {
                                ...
                                executeEvent(routeDefinition.events, 'onResponseError', [responseError, context]);
                            }
                        }

                        if (err) {

                            executeEvent(routeDefinition.events, 'onError', [err, context]);
                        }

                        executeEvent(routeDefinition.events, 'onRequestEnd', [err, context]);
